...
void setBackground()
{
  ...
  if (hour(localTime) < 2 || hour(localTime) > 19)
  { // Yo
    drawArrayJpeg(yo, sizeof(yo), 0, 0);
    light_dark = false;
  }
  else if (hour(localTime) < 8)
  { // Aamu
    drawArrayJpeg(aamu, sizeof(aamu), 0, 0);
    light_dark = true;
  }
  else if (hour(localTime) < 14)
  { // Päivä
    drawArrayJpeg(paiva, sizeof(paiva), 0, 0);
    light_dark = true;
  }
  else if (hour(localTime) < 20)
  { // Ilta
...
void printIcon(int x, int y, String name)
{
  ...
  else if (name == "d01d" || name == "d02d" || name == "d03d" || name == "r01d" || name == "r02d" || name == "r03d" || name == "f01d")
  { // 09d
    tft.drawBitmap(x, y, bitmap_09d, 50, 50, color);
  }
  else if (name == "d01n" || name == "d02n" || name == "d03n" || name == "r01n" || name == "r02n" || name == "r03n" || name == "f01d")
  { // 09n
    tft.drawBitmap(x, y, bitmap_09n, 50, 50, color);
  }
  else if (name == "r04d" || name == "r05d" || name == "r06d")
  { // 10d
    tft.drawBitmap(x, y, bitmap_10d, 50, 50, color);
  }
  else if (name == "r04n" || name == "r05n" || name == "r06n")
  { // 10n
    tft.drawBitmap(x, y, bitmap_10n, 50, 50, color);
  }
  else if (name == "t01d" || name == "t02d" || name == "t03d" || name == "t04d" || name == "t05d")
  { // 11d
    tft.drawBitmap(x, y, bitmap_11d, 50, 50, color);
  }
  else if (name == "t01n" || name == "t02n" || name == "t03n" || name == "t04n" || name == "t05n")
  { // 11n
    tft.drawBitmap(x, y, bitmap_11n, 50, 50, color);
  }
  else if (name == "s01d" || name == "s02d" || name == "s03d" || name == "s04d" || name == "s05d" || name == "s06d")
  { // 13d
    tft.drawBitmap(x, y, bitmap_13d, 50, 50, color);
  }
  else if (name == "s01n" || name == "s02n" || name == "s03n" || name == "s04n" || name == "s05n" || name == "s06n")
  { // 13n
    tft.drawBitmap(x, y, bitmap_13n, 50, 50, color);
  }
  else if (name == "a01d" || name == "a02d" || name == "a03d" || name == "a04d" || name == "a05d" || name == "a06d")
  { // 50d
    tft.drawBitmap(x, y, bitmap_50d, 50, 50, color);
  }
  else if (name == "a01n" || name == "a02n" || name == "a03n" || name == "a04n" || name == "a05n" || name == "a06n")
  { // 50n
...
void loop()
{
  ...
  {
    if (millis() >= (timeTemp + 3600000))
    { // 1h = 3600000  // weather/forecast referesh time
      WB.update();
      updateScreen = true;
      timeTemp = millis();
    }
